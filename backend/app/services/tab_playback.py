    ...
    *,
    ...
    for i in range(max(1, int(max_passes))):
        rep = compare_tab_midi_to_reference(
            reference_midi_path, cur, capo=capo, onset_tolerance_sec=onset_tolerance_sec
        )
        ...
        nxt = nudge_note_events_toward_reference(
            cur, reference_midi_path, capo=capo, onset_tolerance_sec=onset_tolerance_sec
        )
...
def write_tab_compare_artifacts(
    ...
    *,
    ...
    report["compare_before_refine"] = compare_tab_midi_to_reference(
        reference_midi_path, note_events, capo=capo
    )
    report["compare_after_export"] = compare_tab_midi_to_reference(
        reference_midi_path, final_notes, capo=capo
    )
